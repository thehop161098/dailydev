...
class ProcessCrawl implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        Log::info('Info data: ' . json_encode($posts['data']['page']['pageInfo']) . '');

        $data = $posts['data']['page']['edges'];
        $hasNextPage = $posts['data']['page']['pageInfo']['hasNextPage'];
        $endCursor = $posts['data']['page']['pageInfo']['endCursor'];
        ...
        foreach ($data as $edge) {
            ...
            $isCheck = DB::table('posts')->where('post_id', $post['id'])->first();
            ...
            if (empty($isCheck)) {
                ...
                Log::info('' . $post['id'] . ' insert posts in mysql successfully!');

                DB::connection('mongodb')->table('posts')->insert($insertData);
                Log::info('' . $post['id'] . ' insert posts in mongodb successfully!');

            } else {
                ++$flag;
                Log::info(''.$post['id'].' article have already insert ' . $flag . '');
            }
        }
