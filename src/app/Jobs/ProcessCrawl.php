...
class ProcessCrawl implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($data as $edge) {
            ...
            $isCheck = DB::table('posts')->where('post_id', $post['id'])->exists();
            ...
            if (!$isCheck) {
                ...
                ++$flag;
                Log::info($post['id'] . ' article has already been inserted ' . $flag);
            }
        ...
        if (!empty($insertData)) {
            ...
            Log::info('Inserted ' . count($insertData) . ' posts in mysql successfully!');
            DB::connection('mongodb')->table('posts')->insert($insertData);
            Log::info('Inserted ' . count($insertData) . ' posts in mongodb successfully!');
        }
